case  $1 in
  fibers)
    echo "GET http://localhost:8080/loom/demo" | vegeta attack -name=l-threads -duration=10s -timeout=60s -rate=10 | tee "$1".bin | vegeta report && cat "$1".bin | vegeta plot > "$1".html
      ;;
  threads)
